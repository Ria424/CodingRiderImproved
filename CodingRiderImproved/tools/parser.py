...
from ..tools.update import Updater
...
class Parser:
    ...
    def request(self, deviceType, dataType, repeat, interval):
        ...
        for _ in range(repeat):
            drone.send_request(deviceType, dataType)
            sleep(interval)


    def command(self, command_type: CommandType, option: int = 0):
        ...
        # 데이터 요청
        drone.send_command(DeviceType.Base, DeviceType.Drone, command_type, option)
    ...
    def control(self, roll, pitch, yaw, throttle, timeMs):
        ...
        # 데이터 요청
        drone.send_control_while(roll, pitch, yaw, throttle, timeMs)
    ...
    def controlPosition(self, x, y, z, velocity, heading, rotationalVelocity):
        ...
        # 데이터 요청
        drone.send_control_position(x, y, z, velocity, heading, rotationalVelocity)
        sleep(0.1)
    ...
    def buzzer(self, target: DeviceType, hz: int, time: int):
        # drone = Drone(True, True, True, True, True)
        drone = Drone()
        if not drone.open():
            print(Fore.RED + "* Error : Unable to open serial port." + Style.RESET_ALL)
        ...
        data.value = hz
        data.time = time
        drone.send(DataType.Buzzer, Buzzer.getSize(), DeviceType.Base, target, data)
